...
//                logger.debug("### response = " + response);
    ...
    public void computeAndSaveRegMult(BlsCenterModel blsCity, List<SeriesModel> blsSeriesArr) throws SWCException {
        ...
        blsCity.setRegionalFactor(C0_INTERCEPT
                + (C1_READY_MIX * blsVarDict.get(validSeriesIDs[0]))
                + (C2_TRACTOR_SHOVEL * blsVarDict.get(validSeriesIDs[1]))
                + (C3_ENERGY * blsVarDict.get(validSeriesIDs[2]))
                + (C4_FUEL_UTILS * blsVarDict.get(validSeriesIDs[3])));
